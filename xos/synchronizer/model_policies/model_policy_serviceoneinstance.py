...
class ServiceOneInstancePolicy(Policy):
    ...
    def handle_update(self, service_instance):
        if not service_instance.compute_instance:
            ...
            name="serivceone-%s" % service_instance.id
            ...
            cfmap = KubernetesConfigMap(name="serivceone-map-%s" % service_instance.id,
            ...
            secret = KubernetesSecret(name="serivceone-secret-%s" % service_instance.id,
                                      trust_domain=slice.trust_domain,
                                      data=json.dumps(data))
            secret.save()
